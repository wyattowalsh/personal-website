...
$$
y_{i} = \beta_{0} + \beta_{1}x_{i,1} + \beta_{2}x_{i,2} + \dots + \beta_{p}x_{i,p} + \epsilon_{i} \quad \forall i \in \{1, \dots, n\}
\\
\text{where:}
\\
...
\\
\epsilon_{i} \text{ is an error term with a mean of zero and finite variance}
$$
...
\\
y \text{ is a response vector } [y_1, y_2, \dots, y_n]^T \text{ of length } n
\\
X \text{ is an } n \times (p + 1) \text{ design matrix of features } [1, X_1, X_2, \dots, X_p]
\\
\beta \text{ is a length } (p + 1) \text{ coefficient vector } [\beta_0, \beta_1, \beta_2, \dots, \beta_p]
$$
...
$$
\hat{y_i} = \sum_{j=1}^{p} x_{i,j} \hat{\beta_j}, \text{ a model prediction of sample } i, \text{ has an error (or residual) defined as: } y_i - \hat{y_i}
$$

Thus, in vector notation, total model error across all predictions can be found as:

$$
\hat{y} = X \hat{\beta}, \text{ all sample model predictions, has an associated total model error: } ||y - \hat{y}||_2^2
$$
...
$$
\sum_{i=1}^{n}(y_{i}-\hat{y_{i}})^{2}=\sum_{i=1}^{n}(y_{i}-\sum_{j=1}^{p}x_{i,j}\hat{\beta}_{j})^{2}
$$
...
$$
\hat{\beta} = \arg \min_{\beta} L(\beta) = \arg \min_{\beta} \frac{1}{2n} ||y - X \beta||_2^2
$$
...
$$
\beta^T(2X^TX)\beta = 2(X\beta)^TX\beta = 2||X\beta||_2^2 \ge 0 \quad \forall \beta
$$
...
<script src="https://gist.github.com/wyattowalsh/75a3ea1df349c6a3598839d6f042b9e6.js"></script>
