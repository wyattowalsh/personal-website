...
Welcome to the final part of a three-part deep-dive on **regularized linear regression modeling**! In [**part one**](/blog/posts/regularized-linear-regression-models-pt1), linear modeling was established with the derivation of OLS showing how to solve for model coefficients to make predictions of the response given new feature data. Next, in [**part two**](/blog/posts/regularized-linear-regression-models-pt2), **_overfitting_** issues of the OLS model were discussed and **_Ridge Regression_** was presented as a technique to help reduce overfitting through regularization. Building off the same concept as Ridge Regression, **_the Lasso_** and **_the Elastic Net_** are now presented. The series concludes with general considerations of use cases for the techniques presented.
...
\begin{equation*}
\begin{aligned}
\hat{\boldsymbol{\beta}} &= \underset{\boldsymbol{\beta}}{\arg\min} \, L(\boldsymbol{\beta}) \\[0.5em]
&= \underset{\boldsymbol{\beta}}{\arg\min} \left\{ \frac{1}{2n} \|\mathbf{y} - \mathbf{X}\boldsymbol{\beta}\|_2^2 + \lambda \|\boldsymbol{\beta}\|_1 \right\}
\end{aligned}
\end{equation*}
$$
...
$$
\begin{equation*}
\beta_j = S(\beta_j^*, \lambda) = \begin{cases}
\beta_j^* - \lambda & \text{if } \beta_j^* > 0 \text{ and } \lambda < |\beta_j^*| \\[0.5em]
\beta_j^* + \lambda & \text{if } \beta_j^* < 0 \text{ and } \lambda < |\beta_j^*| \\[0.5em]
0 & \text{if } \lambda \geq |\beta_j^*|
\end{cases}
\end{equation*}
$$
...
$$
\begin{equation*}
\sum_{i=1}^{n} x_{i,j} r_i = \langle \mathbf{x}_j, \mathbf{y} \rangle - \sum_{k: |\beta_k| > 0} \langle \mathbf{x}_j, \mathbf{x}_k \rangle \beta_k
\end{equation*}
$$
...
$$
\begin{equation*}
\lambda_{\text{max}} = \frac{\max_l |\langle \mathbf{x}_l, \mathbf{y} \rangle|}{n}
\end{equation*}
$$
...
One possible way to implement **_pathwise coordinate descent_** for **_the Lasso_** (with options for tuning the convergence tolerance, path length, and returning the path) is:
...
\begin{equation*}
\begin{aligned}
L(\boldboldsymbol{\beta}) &= \|\mathbf{y} - \mathbf{X}\boldboldsymbol{\beta}\|_2^2 + \lambda \left( (1-\alpha) \frac{1}{2} \|\boldboldsymbol{\beta}\|_2^2 + \alpha \|\boldboldsymbol{\beta}\|_1 \right), \\[0.5em]
&\text{with tuning parameters } \lambda \geq 0, \, 0 \leq \alpha \leq 1
\end{aligned}
\end{equation*}
$$
...
\begin{equation*}
\begin{aligned}
\hat{\boldboldsymbol{\beta}} &= \underset{\boldboldsymbol{\beta}}{\arg\min} \, L(\boldboldsymbol{\beta}) \\[0.5em]
&= \underset{\boldboldsymbol{\beta}}{\arg\min} \left\{ \frac{1}{2n} \|\mathbf{y} - \mathbf{X}\boldboldsymbol{\beta}\|_2^2 + \lambda \left[(1-\alpha)\frac{1}{2}\|\boldboldsymbol{\beta}\|_2^2 + \alpha\|\boldboldsymbol{\beta}\|_1\right] \right\}
\end{aligned}
\end{equation*}
$$
...
$$
\begin{equation*}
\lambda_{\text{max}} = \frac{\max_l |\langle \mathbf{x}_l, \mathbf{y} \rangle|}{n\alpha}
\end{equation*}
$$
...
If you are just now beginning the series, make sure to check out [**part one**](/blog/posts/regularized-linear-regression-models-pt1) and [**part two**](/blog/posts/regularized-linear-regression-models-pt2)!
