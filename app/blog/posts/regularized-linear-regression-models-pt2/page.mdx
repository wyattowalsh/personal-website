...
$$
L(β) = ||y - Xβ||₂² + λ||β||₂²  \quad \text{with tuning parameter } λ ≥ 0
$$
...
$$\beta^{T}(X^{T}X+\lambda I)\beta=(X\beta)^{T}X\beta+\lambda\beta^{T}\beta=||X\beta||_{2}^{2}+\lambda||\beta||_{2}^{2}>0\quad\forall\beta\ne0$$
