...
This form of regression is also known as **_Tikhonov Regularization_** and modifies the OLS loss function from part one (Equation #7) with the addition of an **_L₂_** penalty with an associated tuning parameter, **_λ_**. This loss function can be described using vector notation as:

$$
\begin{equation*}
L(\boldsymbol{\beta}) = \|\mathbf{y} - \mathbf{X}\boldsymbol{\beta}\|_2^2 + \lambda\|\boldsymbol{\beta}\|_2^2, \quad \text{with } \lambda \geq 0
\end{equation*}
$$
...
$$
\begin{equation*}
\hat{\boldsymbol{\beta}}_{\text{ridge}} = \underset{\boldsymbol{\beta}}{\arg\min} \left\{ \frac{1}{2n} \|\mathbf{y} - \mathbf{X}\boldsymbol{\beta}\|_2^2 + \lambda\|\boldsymbol{\beta}\|_2^2 \right\}
\end{equation*}
$$
...
$$
\begin{equation*}
\mathbf{H} = 2\mathbf{X}^\top\mathbf{X} + 2\lambda \mathbf{I}
\end{equation*}
$$
...
&= (\mathbf{X}\boldsymbol{\beta})^\top\mathbf{X}\boldsymbol{\beta} + \lambda\boldsymbol{\beta}^\top\boldsymbol{\beta} \\[0.5em]
&= \|\mathbf{X}\boldsymbol{\beta}\|_2^2 + \lambda\|\boldsymbol{\beta}\|_2^2 > 0, \quad \forall \boldsymbol{\beta} \neq \mathbf{0}
\end{aligned}
\end{equation*}
$$
...
\hat{\beta}_0 \\ 
\hat{\boldsymbol{\beta}}
\end{bmatrix*} = \underset{\beta_0, \boldsymbol{\beta}}{\arg\min} \left\| 
\begin{bmatrix*}[c] 
\mathbf{y} \\ 
\mathbf{0}
\end{bmatrix*} - 
...
If you have not already, make sure to check out [**part one**!](/blog/posts/regularized-linear-regression-models-pt1)

Continue on to [**part three**](/blog/posts/regularized-linear-regression-models-pt3) to learn about **_the Lasso_** and **_the Elastic Net_**, the last two regularized linear regression techniques!
