...
<SwmSnippet path="/lib/services/backend.ts" line="1">
...
class BackendService {
  ...
  private async processAllPosts(): Promise<PostMetadata[]> {
    // Change the glob pattern to match your MDX files location
    ...
    const posts = await Promise.all(files.map(async (file) => {
      ...
      const slug = file.replace(/^app\/blog\/posts\/|\.mdx$/g, '')
  ...
  private async processPost(file: string, slug: string, gitData: any): Promise<PostMetadata> {
    ...
    return {
      ...
      tags: data.tags || [],
      image: data.image,
      created: data.created || gitData.created,
...
<SwmSnippet path="/scripts/index.ts" line="1">
