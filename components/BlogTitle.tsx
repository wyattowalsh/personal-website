...
import { 
    Code, Binary, Terminal, Brain, Sparkles, CircuitBoard,
    Cpu, Database, Cloud, Network, Github, Chrome,
    Command, Server, Share2, Workflow
} from "lucide-react";
...
const FloatingIcon = ({ icon: Icon, containerSize }) => {
    const iconSize = 16;

    const x = useMotionValue(Math.random() * containerSize.width - containerSize.width / 2);
    const y = useMotionValue(Math.random() * containerSize.height - containerSize.height / 2);
...
const GlitchText = ({ children }) => (
  ...
  >
    <span 
      className="relative inline-block glitch-base"
      data-text={children}
    >
      {/* Base text */}
      <span className={cn(
                ...
                >
                    ...
                    {containerSize.width > 0 &&
                        icons.map((Icon, index) => (
                            <FloatingIcon key={index} icon={Icon} containerSize={containerSize} />
                        ))}
