...
### Studio Auth Required (if `/studio` sign-in is enabled)

| Variable | Description | Example |
| :--- | :--- | :--- |
| `AUTH_SECRET` | Auth.js signing/encryption secret | `openssl rand -base64 32` |
| `AUTH_URL` | Canonical external auth URL (no trailing slash) | `https://w4w.dev` |
| `AUTH_GOOGLE_ID` | Google OAuth client ID | `<google-client-id>` |
| `AUTH_GOOGLE_SECRET` | Google OAuth client secret | `<google-client-secret>` |
| `AUTH_GITHUB_ID` | GitHub OAuth client ID | `<github-client-id>` |
| `AUTH_GITHUB_SECRET` | GitHub OAuth client secret | `<github-client-secret>` |

Auth runtime also accepts these aliases for compatibility: `NEXTAUTH_SECRET`, `NEXTAUTH_URL`, `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `GITHUB_CLIENT_ID`, `GITHUB_CLIENT_SECRET`.

OAuth callback URIs must exactly match your canonical domain:

- Google redirect URI: `https://w4w.dev/api/auth/callback/google`
- GitHub callback URL: `https://w4w.dev/api/auth/callback/github`

If you serve both apex and `www`, register the same callback host in providers and `AUTH_URL` to avoid mismatch failures.

#### OAuth Login Troubleshooting

If login fails with provider `redirect_uri_mismatch` or NextAuth `error=Configuration`, verify:

- **Google**: Authorized redirect URIs include `https://w4w.dev/api/auth/callback/google` (and `https://www.w4w.dev/api/auth/callback/google` if serving `www`).
- **GitHub**: Authorization callback URL uses the canonical host with `/api/auth/callback/github`.
- **Host parity**: `AUTH_URL` matches the canonical host and the host used in provider callback settings.
