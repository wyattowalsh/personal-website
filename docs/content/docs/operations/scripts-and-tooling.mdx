...
| :--- | :--- | :--- |
| `preprocess` | `tsx --conditions react-server scripts/index.ts` | Run preprocessing pipeline manually |
| `predev` | `NODE_ENV=development pnpm preprocess` | Auto-run before dev (development mode) |
| `dev` | `DEBUG=true LOG_LEVEL=debug next dev --turbopack --port 3458` | Start development server with debug logging |
| `prebuild` | `NODE_ENV=production pnpm preprocess` | Auto-run before build (production mode) |
| `build` | `next build` | Production build |
| `start` | `next start --port 3458` | Start production server |
| `lint` | `eslint .` | Run ESLint on all files |
| `typecheck` | `tsc --noEmit` | Run TypeScript type checking |
| `test` | `vitest run` | Run Vitest suite |
| `test:subtitle-geometry` | `playwright test tests/subtitle-geometry.spec.ts` | Run the homepage subtitle geometry gate |
...
| `validate:blog-manager` | `tsx scripts/validate-blog-manager.ts` | Validate the blog manager skill metadata and files |
| `generate-icons` | `tsx scripts/generate-icons.ts` | Generate icon assets |
...
```bash
DEBUG=true LOG_LEVEL=debug next dev --turbopack --port 3458
```
