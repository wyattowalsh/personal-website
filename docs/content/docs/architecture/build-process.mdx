...
**Flow:**
1. `pnpm dev` → triggers `predev` → runs `preprocess` script → starts Next.js dev server
2. `pnpm build` → triggers `prebuild` → runs `preprocess` script → builds production bundle
...
**Example output:**
```
ℹ [INFO] → Mode: Production
↳ [PROGRESS] → ████████████████████ 100.0% | Processing content/posts/personal-website/index.mdx
✓ [SUCCESS] → Preprocessing completed successfully
⏱ [TIME] → Total preprocessing time: 1234.56ms
📊 [MEMORY] → heapUsed: 45.2MB, external: 1.8MB
```
