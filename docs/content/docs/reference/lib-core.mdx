...
Post metadata without full content, word count, or adjacency info (lighter weight).

```typescript
type PostMetadata = Omit<Post, 'content' | 'wordCount' | 'adjacent'>;
```
...
**Output format:**
- Logs are prefixed with level tags (`[INFO]`, `[WARN]`, `[ERROR]`, etc.)
- Structured data is passed as a second argument to console methods
- Error stack traces are logged via `console.error`
